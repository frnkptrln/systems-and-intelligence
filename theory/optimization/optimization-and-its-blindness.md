...
## 2. Local optimization is globally blind `[HYPOTHESIZED]`, resting on a `[FOUNDATIONAL ASSUMPTION]`
...
**This is where the two spines meet.** The *local blindness* that [The Generator Question](../core/the-generator-question.md) names as a precondition for emergence ([Manifesto Claim 2](../core/emergence-manifesto-v1.3.md)) is, read dynamically, the reason an optimizing system cannot see its own trajectory toward a boundary. The inverse problem (*recover the generator*) and the viability problem (*survive the optimizer*) are the same blindness asked in two directions. The blindness is structural, not a deficit of intelligence — no firing neuron, no flocking bird, no profit-maximizing firm, no reward-maximizing policy computes the macro-trajectory it is part of. More intelligence does not lift it. (See §4 — it makes it worse.)
