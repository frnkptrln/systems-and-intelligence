"""Minimal active-identification witness.

Two linear-Gaussian causal generators induce the same observational joint
Gaussian over (X, Y), but different interventional predictions under do(X=x).
The script quantifies expected information gain about generator identity.

This is a toy identifiability experiment, not a reproduction of MDA.
"""
...
def observationally_equivalent() -> bool:
    """Return whether the two observational Gaussian covariance matrices match."""
...
def information_gain_bits(x: float) -> float:
    """Expected information gain I(G;Y | do(X=x)) for equal model priors.
    ...
    def integrand(y: float) -> float:
        pa = interventional_pdf_a(y, x)
        pb = interventional_pdf_b(y, x)
        mixture = 0.5 * (pa + pb)

        value = 0.0
        if pa > 0.0:
            value += 0.5 * pa * math.log2(pa / mixture)
        if pb > 0.0:
            value += 0.5 * pb * math.log2(pb / mixture)
        return value
...
def posterior_probability_a(x: float, y: float) -> float:
    """P(G_A | y, do(X=x)) under equal prior probability for both models."""
