"""Agentic diagnostic dashboard for four selected toy statistics.

This file is retained from the early "Extended SII" work, but its interpretation
has been corrected. It plots four quantities labelled P, R, A, and Δ-Kohärenz
(Ω) for the constructed agents.
...
def compute_agent_sii(agent, n_sessions: int = 100, dc_config: dict | None = None):
    """Run a toy agent and compute four selected dashboard statistics.

    P: cosine alignment between hashed input/response embeddings.
    R: inverse variability of successive self-representation changes.
    A: ratio of second-half to first-half adjacent-representation consistency.
    Ω: the suite's Δ-Kohärenz temporal statistic.

    The historical P/R/A names are kept for compatibility. Their definitions
    here are task-specific proxies, not validated measures of prediction,
    regulation, adaptation, or intelligence in general.
    """
    ...
    for _ in range(n_sessions):
        ...
        if norm_i > 0 and norm_r > 0:
            ...
            sim = 0.0
        alignment_scores.append(max(0, sim))

    # P: local input/response embedding-alignment proxy.
    P = float(np.mean(alignment_scores)) if alignment_scores else 0.0

    # R: local representation-stability proxy.
    if len(reps) > 10:
        distances = []
    ...
    # A: local before/after consistency-ratio proxy.
    if len(reps) > 20:
        ...
        first_consistency = float(
            np.mean(
                [
                    ...
                    / (
                        np.linalg.norm(first_half[i])
                        * np.linalg.norm(first_half[i + 1])
                        + 1e-10
                    )
        ...
        second_consistency = float(
            np.mean(
                [
                    ...
                    / (
                        np.linalg.norm(second_half[i])
                        * np.linalg.norm(second_half[i + 1])
                        + 1e-10
                    )
...
def radar_chart_4d(ax, labels, values_dict, title=""):
    """Draw the four selected statistics as a radar/spider chart."""
    N = len(labels)
...
def run_dashboard(config: dict | None = None):
    """Run the four-statistic Agentic diagnostic dashboard."""
    print("\n" + "=" * 60)
    print("  Agentic Diagnostic Dashboard — P* / R* / A* / Ω")
    print("=" * 60)
    print("\nRunning toy agent simulations...\n")
    ...
    fig.suptitle(
        "Agentic Toy Diagnostics — P* / R* / A* / Ω",
        fontsize=14,
        color="#e0e0ff",
        fontweight="bold",
        y=0.98,
    ...
    radar_chart_4d(
        ax_radar,
        ["Alignment (P*)", "Stability (R*)", "Consistency ratio (A*)", "Δ-Kohärenz (Ω)"],
        results,
        title="Selected toy statistics",
    )
    ...
    for name, (P, R, A, omega) in results.items():
        clean = name.replace("\n", " ")
        print(
            f"  {clean:25s}  P*={P:.2f}  R*={R:.2f}  "
            f"A*={A:.2f}  Ω={omega:.2f}"
        )
    print("=" * 60)
    print("\n  Note: all four values are task-specific toy diagnostics.")
    print("  Ω is a temporal statistic, not an IP proxy or development certificate.\n")
