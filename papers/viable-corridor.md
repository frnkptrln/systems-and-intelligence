...
> **Status:** working draft, version 0.6.
> Sections §1–§4 have completed two reviewer passes; §5–§8 are first-drafted (not yet reviewer-passed). Figure 1 is generated by `lab/tools/viable_corridor.py`; the simulation and the P1/P2/P3/P8 evidence figures of Appendix C are generated by `simulation-models/alignment-and-veto/teo-civilization/teo_simulation.py`. The substrate-veto modeling decision (v0.4) is resolved (v0.5: canonical dissipation tracks raw throughput, so the substrate constraint binds endogenously; §2.5, §6.1). v0.6 adds the central positive result (Appendix C.4): the three *state* axes are dynamically separable, but **capability is a shared driver that loads onto two constraints at once**, so single-axis interventions cannot keep a high-capability system viable — the model-internal content of "constraint architecture dominates capability" (P8) and of the framework's single-axis-insufficiency thesis (§7.1). Appendix A (derivations) is pending; references are consolidated; full bibliographic entries are pending. **This document is not yet submission-ready.**
...
**P8 (Constraint architecture dominates capability).** The framework predicts that, holding constraint architecture fixed, increasing per-agent capability (model scale) does *not* move a system into the corridor — and may move it out, by increasing $f_i^{(0)}$ and hence entropy production. *Test:* vary agent capability and constraint strength independently in simulation; measure in-corridor fraction. *Falsification:* capability alone (with fixed $\gamma$, $K$) reliably produces viability — which would refute the central alignment corollary that "alignment is not a per-model property." *Status (Class A confirmation):* this prediction is now confirmed **for the model itself** in Appendix C.4: raising the capability margin $\delta$ at fixed $(\gamma, K, D_{\max})$ drives the system out of the corridor — first through the concentration boundary, then through the substrate boundary — and no single-axis strengthening (more $\gamma$ alone, or more $D_{\max}$ alone) rescues a high-capability system; only raising *both* together restores viability. The Class C claim — that the same holds for *real* agent ecologies — remains open.
...
- [x] Appendix C (Numerical evidence for Conjecture 1) — P1 (Lemmas 1–2 verified; substrate caveat), P2 ($\gamma_c \approx 0.49$ matches Eq. 9), P3 (lower-corner geometry); figures generated by the teo-civilization simulation
