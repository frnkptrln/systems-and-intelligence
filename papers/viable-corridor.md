...
> **Status:** working draft, version 0.7.
> Sections §1–§4 have completed two reviewer passes; §5–§8 are first-drafted (not yet reviewer-passed). Figure 1 is generated by `lab/tools/viable_corridor.py`; the Appendix C (Class A) figures by `simulation-models/alignment-and-veto/teo-civilization/teo_simulation.py`; the Appendix D (Class C) figure by `simulation-models/alignment-and-veto/agent-ecology/agent_budget_sim.py`. The substrate-veto modeling decision (v0.4) is resolved (v0.5: canonical dissipation tracks raw throughput, so the substrate constraint binds endogenously; §2.5, §6.1). v0.6 added the central positive result (Appendix C.4): the three *state* axes are dynamically separable, but **capability is a shared driver that loads onto two constraints at once**, so single-axis interventions cannot keep a high-capability system viable (P8; §7.1). v0.7 adds Appendix D: P7 and P8 reproduce in a structurally independent stochastic agent-based ecology — first (synthetic) evidence that the regime claims survive a change of model. Appendix A (derivations) is pending; references are consolidated; full bibliographic entries are pending. **This document is not yet submission-ready.**
