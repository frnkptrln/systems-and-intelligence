...
Ma et al. (2026), [*LongHorizon-Harness: Advancing Long-Horizon Agents for Real-World Tasks*](https://arxiv.org/abs/2608.01964), sharpens the distinction. Its architecture places task state outside the growing interaction context, delegates the next step to a fresh executor, and permits a read-only auditor to advance persistent state only from independently verified environment facts. The relevant contrast is therefore not merely short versus long context, but **self-reported continuity versus verified state transport**.

An executable comparison belongs in the separate research-harness workstream. Here, the paper strengthens the conceptual question without creating a second idea or importing harness infrastructure into this repository.
...
- [Experience Needs an Invalidation Boundary](2026-08-22-experience-needs-an-invalidation-boundary.md)
