...
| Generator composition (symbiogenesis bridge) | [static-information-and-living-process.md](../../theory/computation/static-information-and-living-process.md) | `[SPECULATIVE]` — external anchors, no in-repo artifact | Margulis 1967; BFF / computational life (Agüera y Arcas et al. 2024) as illustration only; open question: a minimal composition testbed on the benchmark's equivalence-class machinery |
...
| Chord vs. Arpeggio | [chord-vs-arpeggio-identity.md](../../theory/identity/chord-vs-arpeggio-identity.md) | hypothesis — first toy dissociation measured; functionally deflated | IP metric; Co-Instantiation open problem; [exp5](../../lab/experiments/exp5_availability_dissociation.py): bindings dissociate behaviorally (violations 0.74/0.59/0.03), and sequential-within-one-step is measurably still an arpeggio (12% leak); functional status (§ in home file): joint satisfaction at the commitment boundary — refresh/commit ratio + composition, not simultaneity |
| Identity Persistence (IP) | [lab/metrics/identity_persistence.py](../../lab/metrics/identity_persistence.py) | formalized metric | not yet run on real models; in [exp5](../../lab/experiments/exp5_availability_dissociation.py) its ordering is by design (the sanity floor), not a finding |
