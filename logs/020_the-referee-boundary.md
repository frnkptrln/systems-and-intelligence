...
A bounded generate–evaluate–revise loop was run against three referees. Against a frozen referee, the loop went all-green and its held-out accuracy saturated at the evidence ceiling — ten times the revision budget did not move it beyond that ceiling. When the referee could add tests the loop could not add itself, the ceiling rose and held-out accuracy followed. When the loop could delete failing tests from its own evaluator, the report went from 0.7996 ± 0.0476 to 0.9910 ± 0.0181 (mean ± sd of per-row means over 1,024 evidence rows, each row × all 256 rules) while held-out accuracy did not improve (0.7035 ± 0.0353 to 0.7006 ± 0.0350) — the gap between what the system reports and what it does tripled (0.096 to 0.290). The comparisons use paired proposal streams, so the declared referee property changes while the random proposals do not. None of this required an adversarial proposer; a hill climb plus one permissive permission was enough.

> **Correction, 2026-08-19.** This paragraph originally read "the report went from 0.80 to 0.99 … tripled." Those figures came from the benchmark's v0.1 grid, whose random streams were seeded with the hidden rule (see [LEAKAGE-IMPACT-2026-08-19](../lab/benchmarks/recursive-workbench/LEAKAGE-IMPACT-2026-08-19.md)); the values above are the leak-free v0.2 re-measurement with row count and dispersion stated. The original rounded claims survive re-measurement unchanged.
...
**Agentic coding loops.** A loop that writes code and then writes the tests for that code sits on both sides of the table at once. The toy's prediction is not that such loops fail — it is that their all-green reports are not sufficient evidence of held-out behavior. The fix the measurement suggests is the witness move: held-out tests the loop never sees, spent like a budget, added by the harness at plateaus. That is Arm 3, and it is cheap in this toy — a two-query budget moved held-out accuracy by eleven points (v0.2 grid: 0.8474 → 0.9568, +10.9, identity-exact; the figure was first measured as +10.9 on the withdrawn leak-era grid — see the correction above).
